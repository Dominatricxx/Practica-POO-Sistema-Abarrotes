    ...
    def listar_empleados(self):
        ...
        cursor.execute('''
            SELECT id, nombre, apellido, username,
                strftime('%Y-%m-%d %H:%M', fecha_creacion) as fecha_creacion
            FROM empleados
        ''')
    ...
    def obtener_ventas_por_periodo(self, fecha_inicio):
        ...
        cursor.execute('''
            SELECT v.id, v.folio, 
                ...
                v.total, v.descuento, v.puntos_ganados,
...
async def reporte_ventas_detalle(periodo: str = "todas"):
    ...
    for v in ventas_rows:
        ...
        fecha_original = v[2]
        if 'T' in fecha_original:
            fecha_original = fecha_original.replace('T', ' ')
            if len(fecha_original) > 16:
                fecha_original = fecha_original[:16]
    ...
    return [
        {
            ...
            "apellido": e[2],
            "username": e[3],
            "fecha_creacion": e[4].replace('T', ' ')[:16] if e[4] and 'T' in str(e[4]) else e[4]
        }
        for e in empleados
    ]
