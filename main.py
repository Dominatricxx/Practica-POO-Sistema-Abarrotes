...
async def eliminar_empleado(request: Request):
    try:
        data = await request.json()
        ...
        if total_empleados <= 1:
            conn.close()
            return {"success": False, "message": "No se puede eliminar el unico empleado"}
        
        cursor.execute('SELECT username FROM empleados WHERE id = ?', (empleado_id,))
        empleado = cursor.fetchone()
...
@app.post("/api/clientes/eliminar")
async def eliminar_cliente(request: Request):
    try:
        data = await request.json()
        cliente_id = data.get("id")
        codigo_cliente = data.get("codigo_cliente")
        
        if not cliente_id and not codigo_cliente:
        ...
        cliente_real_id = cliente[0]
        cursor.execute('DELETE FROM clientes WHERE id = ?', (cliente_real_id,))
        conn.commit()
        conn.close()
