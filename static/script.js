...
function mostrarModalAgregarUsuarioBD() {
    document.getElementById('modalSeleccionarTipoUsuario').style.display = 'block';
}

function cerrarModalSeleccionarTipoUsuario() {
    document.getElementById('modalSeleccionarTipoUsuario').style.display = 'none';
}

function mostrarModalBuscarUsuario() {
    document.getElementById('modalBuscarUsuario').style.display = 'block';
    document.getElementById('buscarUsuarioID').value = '';
    document.getElementById('buscarUsuarioNombre').value = '';
    document.getElementById('buscarUsuarioApellido').value = '';
    document.getElementById('buscarUsuarioCorreo').value = '';
    document.getElementById('buscarUsuarioTelefono').value = '';
    document.getElementById('resultadoBusquedaUsuario').innerHTML = '';
}

function cerrarModalBuscarUsuario() {
    document.getElementById('modalBuscarUsuario').style.display = 'none';
}

function mostrarModalEliminarUsuario() {
    document.getElementById('modalEliminarUsuario').style.display = 'block';
    document.getElementById('eliminarUsuarioID').value = '';
    document.getElementById('errorEliminarUsuario').innerHTML = '';
}
...
async function buscarUsuario() {
    ...
    const nombre = document.getElementById('buscarUsuarioNombre').value.trim().toLowerCase();
    const apellido = document.getElementById('buscarUsuarioApellido').value.trim().toLowerCase();
    const correo = document.getElementById('buscarUsuarioCorreo').value.trim().toLowerCase();
    const telefono = document.getElementById('buscarUsuarioTelefono').value.trim();

    if (!id && !nombre && !apellido && !correo && !telefono) {
        document.getElementById('resultadoBusquedaUsuario').innerHTML = '<p style="color: #e53e3e; text-align: center;">Debes llenar al menos un campo</p>';
        return;
    }

    try {
        const respuestaEmpleados = await fetch('/api/empleados/listar');
        const empleados = await respuestaEmpleados.json();

        const respuestaClientes = await fetch('/api/clientes');
        const clientes = await respuestaClientes.json();

        let resultados = [];

        if (id) {
            const empEncontrado = empleados.find(e => e.id.toString() === id);
            if (empEncontrado) {
                resultados.push({ tipo: 'Empleado', id: empEncontrado.id, nombre: empEncontrado.nombre, apellido: empEncontrado.apellido, usuario: empEncontrado.username, fecha: empEncontrado.fecha_creacion, email: 'N/A', telefono: 'N/A', puntos: 'N/A' });
            }
            const cliEncontrado = clientes.find(c => (c.codigo_cliente && c.codigo_cliente.toString() === id) || (c.id && c.id.toString() === id));
            if (cliEncontrado) {
                resultados.push({ tipo: 'Cliente', id: cliEncontrado.codigo_cliente || cliEncontrado.id, nombre: cliEncontrado.nombre, apellido: cliEncontrado.apellido || '', usuario: 'N/A', fecha: cliEncontrado.fecha_registro || 'No disponible', email: cliEncontrado.email || 'No registrado', telefono: cliEncontrado.telefono, puntos: cliEncontrado.puntos });
            }
        } else {
            empleados.forEach(e => {
                if ((nombre && e.nombre.toLowerCase().includes(nombre)) ||
                    (apellido && e.apellido.toLowerCase().includes(apellido))) {
                    if ((!nombre || e.nombre.toLowerCase().includes(nombre)) &&
                        (!apellido || e.apellido.toLowerCase().includes(apellido))) {
                        resultados.push({ tipo: 'Empleado', id: e.id, nombre: e.nombre, apellido: e.apellido, usuario: e.username, fecha: e.fecha_creacion, email: 'N/A', telefono: 'N/A', puntos: 'N/A' });
                    }
                }
            });
            clientes.forEach(c => {
                const nombreCliente = (c.nombre || '').toLowerCase();
                const apellidoCliente = (c.apellido || '').toLowerCase();
                const emailCliente = (c.email || '').toLowerCase();
                const telefonoCliente = c.telefono || '';

                let coincide = true;
                if (nombre && !nombreCliente.includes(nombre)) coincide = false;
                if (apellido && !apellidoCliente.includes(apellido)) coincide = false;
                if (correo && !emailCliente.includes(correo)) coincide = false;
                if (telefono && !telefonoCliente.includes(telefono)) coincide = false;

                if (coincide && (nombre || apellido || correo || telefono)) {
                    resultados.push({ tipo: 'Cliente', id: c.codigo_cliente || c.id, nombre: c.nombre, apellido: c.apellido || '', usuario: 'N/A', fecha: c.fecha_registro || 'No disponible', email: c.email || 'No registrado', telefono: c.telefono, puntos: c.puntos });
                }
            });
        }

        if (resultados.length === 0) {
            document.getElementById('resultadoBusquedaUsuario').innerHTML = '<p style="text-align: center; color: #999;">No se encontraron usuarios</p>';
            return;
        }

        let html = '<table style="width: 100%; border-collapse: collapse; font-size: 0.85em;">' +
            '<thead><tr style="background: #667eea; color: white;">' +
            '<th style="padding: 8px;">Tipo</th><th style="padding: 8px;">ID</th><th style="padding: 8px;">Nombre</th><th style="padding: 8px;">Apellido</th><th style="padding: 8px;">Email/Usuario</th><th style="padding: 8px;">Telefono</th><th style="padding: 8px;">Puntos</th><th style="padding: 8px;">Fecha</th>' +
            '</tr></thead><tbody>';
        ...
        resultados.forEach((r, i) => {
            html += '<tr style="background: ' + (i % 2 === 0 ? '#f8f9fa' : 'white') + ';">' +
                ...
                '<td style="padding: 8px;">' + escapeHtml(r.nombre) + '</td>' +
                '<td style="padding: 8px;">' + escapeHtml(r.apellido) + '</td>' +
                '<td style="padding: 8px;">' + (r.tipo === 'Empleado' ? r.usuario : r.email) + '</td>' +
                '<td style="padding: 8px;">' + escapeHtml(r.telefono) + '</td>' +
                '<td style="padding: 8px; text-align: center;">' + r.puntos + '</td>' +
                '<td style="padding: 8px;">' + r.fecha + '</td>' +
                '</tr>';
        });

        html += '</tbody></table>';
        document.getElementById('resultadoBusquedaUsuario').innerHTML = html;
    } catch (error) {
        document.getElementById('resultadoBusquedaUsuario').innerHTML = '<p style="color: #e53e3c; text-align: center;">Error al buscar usuarios</p>';
    }
}

async function eliminarUsuario() {
    const id = document.getElementById('eliminarUsuarioID').value.trim();
    const errorDiv = document.getElementById('errorEliminarUsuario');

    if (!id) {
        errorDiv.innerHTML = 'Ingresa un ID de usuario';
        return;
    }
    ...
    try {
        ...
        const empleadoEncontrado = empleados.find(e => e.id.toString() === id);
        const clienteEncontrado = clientes.find(c => (c.codigo_cliente && c.codigo_cliente.toString() === id) || (c.id && c.id.toString() === id));

        if (!empleadoEncontrado && !clienteEncontrado) {
            errorDiv.innerHTML = 'No se encontro ningun usuario con ese ID';
            return;
        }

        if (empleadoEncontrado) {
            if (empleados.length <= 1) {
                errorDiv.innerHTML = 'No se puede eliminar el unico empleado registrado';
                return;
            }

            if (empleadoEncontrado.username === 'admin') {
                errorDiv.innerHTML = 'No se puede eliminar al administrador principal';
                return;
            }

            const textoRolActual = document.getElementById('rolActual').textContent || document.getElementById('rolActual').innerText || '';
            const nombreEmpleadoActual = textoRolActual.replace('Empleado: ', '').split(' | ')[0].trim().toLowerCase();
            const nombreEmpleadoEncontrado = empleadoEncontrado.nombre.toLowerCase();

            if (nombreEmpleadoActual === nombreEmpleadoEncontrado) {
                errorDiv.innerHTML = 'No puedes eliminarte a ti mismo';
                return;
            }

            if (!confirm('Estas seguro de eliminar al empleado ' + empleadoEncontrado.nombre + ' ' + empleadoEncontrado.apellido + '?')) return;

            const conn = await fetch('/api/empleados/eliminar', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: empleadoEncontrado.id })
            });
            ...
            if (result.success) {
                ...
                errorDiv.innerHTML = result.message || 'Error al eliminar empleado';
            }
        } else if (clienteEncontrado) {
            if (!confirm('Estas seguro de eliminar al cliente ' + clienteEncontrado.nombre + ' ' + (clienteEncontrado.apellido || '') + '?')) return;

            const conn = await fetch('/api/clientes/eliminar', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: clienteEncontrado.id || clienteEncontrado.codigo_cliente, codigo_cliente: clienteEncontrado.codigo_cliente })
            });
            const result = await conn.json();
            ...
            if (result.success) {
                ...
                errorDiv.innerHTML = result.message || 'Error al eliminar cliente';
            }
        }
    } catch (error) {
        errorDiv.innerHTML = 'Error al procesar la solicitud';
    }
}
